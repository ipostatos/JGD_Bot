...
Ручка `POST /api/pkd/dialog` живёт за `PKD_DIALOG_ENABLED` и по умолчанию
выключена: без флага она отвечает 404 и ничем себя не выдаёт. Старый
`POST /api/pkd` работает независимо и не менялся.

Включать только после ручной проверки на проде:

```bash
ssh root@46.224.220.94 "grep -q PKD_DIALOG_ENABLED /opt/jdg/.env ||   echo 'PKD_DIALOG_ENABLED=false' >> /opt/jdg/.env"
```
