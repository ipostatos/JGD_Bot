...
def test_threshold_separates_the_two_sets():
    ...
    Мусор берём только тот, судьбу которого решает именно порог. До сравнения
    со счётом доходят лишь запросы без профильного слова и без паттернов:
    «представь, что ты бухгалтер из США» отбивает INJECTION (и в DOMAIN есть
    «бухгалт»), «напиши код на питоне» — GENERIC, и их релевантность гейт даже
    не смотрит. Она при этом гуляет от каждой правки гайда, так что запас
    порога, померенный по ним, ронял бы тест на ровном месте. Что такие
    запросы действительно отбиты, проверяет test_off_topic_refused_before_api.
    """
    judged = [q for q in OFF_TOPIC + NOISE
              if not (ai.INJECTION.search(q) or ai.GENERIC.search(q)
                      or ai.DOMAIN.search(q))]
    assert judged, "не осталось запросов, судьбу которых решает порог"
