...
def close_month(month: str, client: Infakt | None = None, profile: Profile | None = None,
    ...
    # 2. KSeF
    no_ksef = [i for i in invoices if not i.get("ksef_number")]
    rep.lines.append(f"KSeF: {len(invoices) - len(no_ksef)}/{len(invoices)} приняты  [api:invoices.ksef_number]")
    for i in no_ksef:
    ...
    revenue_ytd = sum(
        (zl(i["net_price"]) for i in client.invoices() if (i.get("invoice_date") or "").startswith(str(year))),
        Decimal(0),
    )
    ...
    if fee:
        checks += [
            ...
            Check("ZUS zdrowotna", zus_calc.zdrowotna, zl(fee["health_amount_price"]), "api:insurance_fees.health"),
        ...
        rep.lines.append(
            ...
            f"{zus_calc.total} zł, срок {fee['payment_date']}, статус {fee['status']}  [rules+api]"
        )
    else:
    ...
    tax_calc = None
    if prev_fee and prev_fee.get("status") == "paid":
