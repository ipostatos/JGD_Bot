...
LIMITS = {
    "nip": (10, 100, 60),
    # ассистент стоит денег за каждый вызов: три вопроса в минуту — это темп
    # живого человека, а не скрипта. Суточный потолок дублирует квоту в ai.py.
    "ai": (3, 10, 30),
    # диалог — это несколько запросов подряд, и обрывать разговор на третьем
    # вопросе нельзя. Потолок защищает от скриптов, а не от живого человека.
    "pkd_dialog": (20, 200, 200),
}
KEEP = 86400
