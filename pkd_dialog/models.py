...
class Status(str, Enum):
    ...
    # деятельность не сформирована вовсе. Отличать от предыдущего статуса
    # обязательно: «ремонт сайта» — это не «понятая деятельность вне пакета»,
    # а «мы вообще не поняли, чем человек занимается». Иначе аналитика будет
    # уверять, что запрос успешно распознан, а маршрутизировать его некуда
...
class PrimaryStatus(str, Enum):
    ...
    # деятельность одна и код у неё один: выбирать не из чего. Это не выбор
    # движка и не score — просто у альтернатив нет конкурентов, а запрос
    # человека при этом выполнен
