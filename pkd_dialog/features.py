...
# ── как продаёт и как оказывает ───────────────────────────────────────────
# Опт и розница — разные подклассы торговли, и различает их только это.
# «Отдельно» рядом с услугой — сигнал самостоятельной деятельности, а не
# части комплекса: «продаю мебель и ОТДЕЛЬНО собираю» это два направления.
TRADE_WHOLESALE = "trade.wholesale"
TRADE_RETAIL = "trade.retail"
QUALIFIER_SEPARATELY = "qualifier.offered_separately"
...
REGISTRY: dict[str, FeatureKey] = {f.key: f for f in (
    ...
    FeatureKey(TRADE_WHOLESALE, "bool", "продаёт оптом, перепродавцам"),
    FeatureKey(TRADE_RETAIL, "bool", "продаёт в розницу, конечному покупателю"),
    FeatureKey(QUALIFIER_SEPARATELY, "bool",
               "услуга названа отдельной, а не частью комплекса"),
